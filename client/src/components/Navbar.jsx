...
function Navbar() {
  ...
  const { isSignedIn, user } = useUser(); // ✅ FIXED: user added
    ...
    <div className='flex items-center justify-between mx-4 py-3 lg:mx-44'>
      ...
      </Link>
      {
        ...
        ? <div className='flex items-center gap-2 sm:gap-3'>
            ...
            <p className='text-gray-600 max-sm:hidden'>Hi, {user.fullName}</p> {/* ✅ FIXED: safe access */}
            <UserButton />
          </div>
        : <button 
            onClick={() => openSignIn({})}
            className='bg-zinc-800 cursor-pointer text-white flex items-center gap-4 px-4 py-2 sm:px-8 sm:py-3 text-sm rounded-full'>
